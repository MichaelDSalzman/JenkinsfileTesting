def scan(def onFailure) {
  ...
  withSonarQubeEnv {
      ...
      script {
        ...
        List<String> failureDetails = QUALITY_MAP['projectStatus']['conditions'].findAll { it['status'] == 'ERROR' }.collect {
            "${it['metricKey']} is in an error state. Threshold must be ${it.comparator} ${it.errorThreshold}. Actual value is ${it.actualValue}."
          }

            if (qg.status != 'OK') {
          onFailure("Pipeline aborted due to sonar quality gate failure: ${qg.status} - ${failureDetails}")
            }
      }
  }
}
